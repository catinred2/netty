 ...
 * {@link TimerTask} on time. {@link EasyHashedWheelTimer}, on every tick, will
 ...
 * 
 * {@link EasyHashedWheelTimer} maintains a data structure called 'wheel'. To put
 ...
 * 
 * {@link EasyHashedWheelTimer} creates a new thread whenever it is instantiated
 ...
 * 
 * {@link EasyHashedWheelTimer} is based on <a
 ...
 * >here</a>.
 */
public class EasyHashedWheelTimer implements EasyTimer {
    
    static final InternalLogger logger = InternalLoggerFactory.getInstance(EasyHashedWheelTimer.class);
    
    private static final ResourceLeakDetector<EasyHashedWheelTimer> leakDetector = new ResourceLeakDetector<EasyHashedWheelTimer>(EasyHashedWheelTimer.class, 1,
        Runtime.getRuntime().availableProcessors() * 4);
    
    private static final AtomicIntegerFieldUpdater<EasyHashedWheelTimer> WORKER_STATE_UPDATER;
    static {
        AtomicIntegerFieldUpdater<EasyHashedWheelTimer> workerStateUpdater = PlatformDependent.newAtomicIntegerFieldUpdater(EasyHashedWheelTimer.class, "workerState");
        if (workerStateUpdater == null) {
            workerStateUpdater = AtomicIntegerFieldUpdater.newUpdater(EasyHashedWheelTimer.class, "workerState");
        }
     ...
     */
    public EasyHashedWheelTimer(long tickDuration, TimeUnit unit) {
     ...
     */
    public EasyHashedWheelTimer(long tickDuration, TimeUnit unit, int ticksPerWheel) {
     ...
     */
    public EasyHashedWheelTimer(ThreadFactory threadFactory) {
     ...
     * @throws IllegalArgumentException if {@code tickDuration} is <= 0
     */
    public EasyHashedWheelTimer(ThreadFactory threadFactory, long tickDuration, TimeUnit unit) {
     ...
     *             {@code ticksPerWheel} is <= 0
     */
    public EasyHashedWheelTimer(ThreadFactory threadFactory, long tickDuration, TimeUnit unit, int ticksPerWheel) {
    ...
    public Set<Timeout> stop() {
        if (Thread.currentThread() == workerThread) {
            throw new IllegalStateException(EasyHashedWheelTimer.class.getSimpleName() + ".stop() cannot be called from " + TimerTask.class.getSimpleName());
        }
    ...
    @Override
    public Timeout newTimeout(EasyTimerTask task, long delay, TimeUnit unit) {
    ...
    private final class Worker implements Runnable {
        ...
        public void run() {
            ...
            while (WORKER_STATE_UPDATER.get(EasyHashedWheelTimer.this) == WORKER_STATE_STARTED);
        ...
        private long waitForNextTick() {
            ...
            for (;;) {
                ...
                try {
                    ...
                    if (WORKER_STATE_UPDATER.get(EasyHashedWheelTimer.this) == WORKER_STATE_SHUTDOWN) {
...
//        private Lock lock = new Lock();
        ...
        private final EasyHashedWheelTimer timer;
        
        private final EasyTimerTask task;
        ...
        HashedWheelTimeout(EasyHashedWheelTimer timer, EasyTimerTask task, long deadline) {
            this.timer = timer;
            this.task = task;
            this.deadline = deadline;
            this.task.hashedWheelTimer = timer;
        }
        
        @Override
        public EasyTimer timer() {
            return timer;
        }
        ...
        public boolean cancel() {
            ...
            // If a task should be canceled we create a new Runnable for this to
            // another queue which will
            // be processed on each tick. So this means that we will have a GC
            // latency of max. 1 tick duration
            // which is good enough. This way we can make again use of our
            // MpscLinkedQueue and so minimize the
            // locking / overhead as much as possible.
            //
            // It is important that we not just add the HashedWheelTimeout
            // itself again as it extends
        ...
        public void expire() {
            ...
            try {
                ...
                Date now = new Date();
                Date nextTime = task.cronExpression.getNextValidTimeAfter(now);
                if (nextTime != null) {
                    long span = nextTime.getTime() - System.currentTimeMillis();
                    if (span < 0) {
                        span = 0;
                    }
                    Timeout timeout=task.hashedWheelTimer.newTimeout(task, span, TimeUnit.MILLISECONDS);
                    TimerUtil.timeoutMap.put(task.getName(), timeout);
                }
